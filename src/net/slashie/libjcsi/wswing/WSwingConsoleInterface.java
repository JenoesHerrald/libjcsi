...
/**
 * Swing interface for input and output. Returns keystrokes as CharKeys. Shows
 * the characters in a Frame
 * 
...
public class WSwingConsoleInterface implements ConsoleSystemInterface, Runnable, ComponentListener {
	...
	/**
	 * Allows for setting the window's name and deploying as a Java WebStart
	 * application.
	 * 
	 * @param windowName
	 * @param sandboxDeploy
	 *            true if intended at a Java Webstart application
	 ...
	 * @param windowName
	 * @param configuration
	 *            A Properties object containing key-value configuration pairs
	 */
	...
	public String input(int l) {
		String ret = "";// the string that the user typed in
		CharKey read = new CharKey(CharKey.NONE);// the key that the user entered
		while (true) {
			while (read.code == CharKey.NONE) {
				// while we diden't get a key try to read one
				read = inkey();

			}
			if (read.isMetaKey()) {
				// don't add meta keys to the resulting string
